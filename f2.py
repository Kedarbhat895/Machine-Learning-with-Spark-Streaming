...
ra=F.udf(f2,T.ArrayType(T.StringType()))
...
remove_url =lambda  s:re.sub(url,'URL',s)
remove_handler = lambda s :re.sub(up,'',s)
remove_hashtag =lambda  s:re.sub(hashtagPattern,'',s)  #remove  hashtags
neat_sequence  =lambda s :re.sub(seqpattern ,replacepattern,s)
remove_extra_spaces=lambda s:re.sub(r'\s+',' ',s,flags=re.I)
remove_qoutes =lambda s:re.sub(r'"','',s)
neat_alpha =lambda  x:'  '.join(re.findall(r'\w+',x))
...
def text_preprocess(df):
		df=df.withColumn("lemmatized1",ra(F.col("lemmatized1")))
		df=df.withColumn("lemmatized2",ra(F.col("lemmatized2")))
